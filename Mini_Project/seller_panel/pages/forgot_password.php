...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($email) && filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        $stmt = $conn->prepare("SELECT Seller_ID FROM seller WHERE Email_Id = ?");
 ...
 <?php if (!empty($message)): ?>
              ...
              <form method="POST">
                <div class="input-group input-group-outline my-3">
                  <input type="email" name="Email_ID" class="form-control" required placeholder="Enter your email">
                </div>
                <div class="text-center">
                  <button type="submit" class="btn bg-gradient-dark w-100 my-4">Confirm</button>
                </div>
