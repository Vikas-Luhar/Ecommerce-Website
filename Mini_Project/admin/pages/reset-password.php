...
if (!isset($_SESSION['reset_email'])) {
    echo "<script>alert('Unauthorized access!'); window.location='sign-in.php';</script>";
    exit();
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($new_password !== $confirm_password) {
        ...
        $stmt = $conn->prepare("SELECT Password FROM admin WHERE Email_ID = ?");
        $stmt->bind_param("s", $email);
        $stmt->execute();
        $stmt->store_result();

        if ($stmt->num_rows > 0) {
            $stmt->bind_result($old_hashed_password);
            $stmt->fetch();

            // Check if new password matches the old password
            if (password_verify($new_password, $old_hashed_password)) {
                $error_message = "⚠️ New password cannot be the same as the old password!";
            } else {
                // Hash the new password
                $hashed_password = password_hash($new_password, PASSWORD_DEFAULT);

                // Update password in the database
                $update_stmt = $conn->prepare("UPDATE admin SET Password=? WHERE Email_ID=?");
                $update_stmt->bind_param("ss", $hashed_password, $email);
                if ($update_stmt->execute()) {
                    session_destroy(); // Clear session
                    header("Location: sign-in.php?success=1"); // Redirect with success message
                    exit();
                } else {
...
?>
  ...
  <!DOCTYPE html>
    ...
    <?php endif; ?>
                <form method="POST" class="container mt-5 p-4 border rounded shadow" style="max-width: 400px;">
